...
def save_detection_image(picam2, folder="detections"):
...
def parse_detections(metadata: dict):
    ...
    for det in last_detections:
        ...
        w, h = x2 - x1, y2 - y1
        label = labels[int(det.category)] if int(det.category) < len(labels) else str(det.category)
        detection_strings.append(f"{label} ({det.conf:.2f}) at ({x1},{y1},{w},{h})")
...
def draw_detections(request, stream="main"):
    ...
    with MappedArray(request, stream) as m:
        for detection in last_detections:
            ...
            cv2.rectangle(m.array, (x1, y1), (x2, y2), (0, 255, 0, 0), thickness=4)
...
if __name__ == "__main__":
    ...
    try:
        while True:
            detection_strings = parse_detections(picam2.capture_metadata())
            if detection_strings:
                print("[INFO] Detections:")
                for det in detection_strings:
                    print(f"  - {det}")
                save_detection_image(picam2)
